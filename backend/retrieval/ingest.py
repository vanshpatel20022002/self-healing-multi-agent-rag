...
def ingest_path(path: Path, store: VectorStore | None = None) -> IngestResult:
    """Ingest a single file or all documents under a directory into ChromaDB."""
    ...
    for file_path in files:
        try:
            ...
            total_chunks += vector_store.add_chunks(chunks, metadatas)
        except Exception:
            skipped.append(str(file_path))
            continue
